...
export function serializePayload(filename, fileBuffer, expirationTime = 0) {
  if (typeof filename !== 'string') {
    throw new OpaqueError('format: filename debe ser una cadena');
  }
  if (!Buffer.isBuffer(fileBuffer)) {
    throw new OpaqueError('format: fileBuffer debe ser un Buffer');
  }
...
export function serializeMultiPayload(filesList, expirationTime = 0) {
  if (!Array.isArray(filesList)) {
    throw new OpaqueError('format: filesList debe ser un arreglo');
  }
  const chunks = [];
  ...
  for (const f of filesList) {
    if (!f || typeof f.relPath !== 'string' || !Buffer.isBuffer(f.content)) {
      throw new OpaqueError('format: archivo en bóveda inválido');
    }
